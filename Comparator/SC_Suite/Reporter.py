...
class Reporter():
    ...
    def report(self, text = '', err = False, newline = True):
        ...
        if len(B) == 0:
            if newline:
                print("{a} SC Suite : {b}".format(a = sign, b = text))
            else:
                print("{a} SC Suite : {b}".format(a = sign, b = text)),            # does not print 'end of line sign' at the end, so one can continue to write to the same line 
        else:
            if newline:
                print("{a} SC Suite > {b}: {c}".format(a = sign, b = B, c = text))
            else:
                print("{a} SC Suite > {b}: {c}".format(a = sign, b = B, c = text)),    # does not print 'end of line sign' at the end, so one can continue to write to the same line 
